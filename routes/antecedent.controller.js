...
module.exports = {
    ...
    update: (req, res) => {
        let headerAuth = req.headers['authorization'];
        let user = jwtUtils.getUserId(headerAuth);
        let antecedentId = req.params.id;
            ...
            (antecedentFound, done) => {
                if (antecedentFound) {
                    antecedentFound.update({
                        ...
                        isSecure: (isSecure ? isSecure : antecedentFound.isSecure),
                        createdBy: (createdBy ? createdBy : antecedentFound.createdBy)
                    }, {
                        where: { id: antecedentFound.id }
                    })
                        .then((antecedentUpdated) => {
                            done(antecedentUpdated);
                        })
                        .catch((err) => {
                            return res.status(500).json({
                                'status': 'Error',
            ...
            (antecedentFound, done) => {
                if (antecedentFound) {
                    models.Antecedent.destroy({
                        ...
                        .then((antecedentDeleted) => {
                            done(antecedentDeleted)
                        })
                        .catch((err) => {
                            return res.status(500).json({
                                'status': 'Error',
